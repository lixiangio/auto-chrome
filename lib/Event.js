...
const { signale, newPromise } = require('./helper');
...
class Event {
   constructor() {
      ...
      // 等待初始标签准备就绪
      this.waitTargetCreated = newPromise()
   ...
   send(method = '', params = {}) {
      ...
      return new Promise((resolve, reject) => {
         ...
         signale.error(`messageId：${id}`, error.message)
      })
   ...
   async 'Target.targetCreated'({ targetInfo }) {
      ...
      if (type === 'page') {
         ...
         if (!this.page.awaitLoading) {
            this.page.awaitLoading = newPromise(8000)
         }
