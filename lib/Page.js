...
const { logger, zPromise } = require('./helper');
...
class Page extends PageEvent {
   ...
   async title() {
      ...
      let { value } = await this.runtime.evaluate(() => document.title)
   ...
   async url() {
      ...
      let { value } = await this.runtime.evaluate(() => window.location.href)
